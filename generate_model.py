...
#     logging.info("Extracting mesh...")
#     meshes = model.extract_mesh(scene_codes, True, resolution=resolution)  # Skip texture baking

#     os.makedirs(output_dir, exist_ok=True)
#     mesh = meshes[0]

#     # === Save mesh as model.txt ===
#     logging.info("Saving mesh as model.txt...")
...
#         logging.warning("No vertex colors found. Using default grey color.")
#         colors = np.ones((len(vertices), 3), dtype=np.float32) * 0.5  # Default grey
#     else:
#         colors = colors[:, :3] / 255.0 if colors.max() > 1.0 else colors[:, :3]

#     with open(txt_path, 'w') as f:
#         f.write("# Vertices (x y z r g b) with r/g/b in range [0.0 - 1.0]\n")
